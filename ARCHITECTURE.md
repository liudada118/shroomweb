...
> 本文档由 Codex 维护。最后更新于：2026-08-27

## 1. 项目概览

`fiber-sensor` 是一个基于 React + Vite + Three.js 的单页交互演示项目，用来展示柔性压力传感器、粒子模型变形和压力点云读出。当前首页以历史文件 `shroom-vision-home-effects.html` 为直接结构基础，保留原顶栏、品牌标题、访问密钥区、四张行业方案卡、反馈入口、响应式布局以及 Canvas 压力点阵；白色 SHROOM 标志以内嵌本地资源加载，避免子目录部署时丢失。GSAP 负责首页分段入场、卡片指针倾斜、系统列表弹窗和动态生成关闭叉，Three.js 继续负责粒子本身的采样与 Morph。首页蘑菇与弹窗右侧预览共用一个常驻 `SystemScenePreview` 实例，打开系统列表时通过 GSAP Flip 将同一个 WebGL 画布移动到弹窗宿主，保留当前粒子位置、renderer 和模型缓存，再在 `shroom.glb / bed.glb / chair3.glb / jiqirenGggg.fbx` 之间左右交替扫动变形；关闭弹窗时会先把同一画布反向 Flip 回首页，并在移动中重新形成蘑菇。首页保持原有信息结构，但把蘑菇宿主扩展为完整视口背景并置于交互内容后方，Three.js 同时绘制低密度远景粒子；访问密钥区、行业卡片和内部模块改成低遮挡轻透材质，使桌面与移动端都能看见连续的蘑菇粒子环境。点击右侧“进入该系统”后，展示布局直接切换到全屏，WebGL 画布由 `ResizeObserver` 按新尺寸重绘而不参与 Flip 缩放；切换前会记录当前粒子中心的真实屏幕投影，切换后用相机视窗偏移和 FOV 补偿接住同一位置与尺寸，再平滑归零到居中构图。返回列表使用同一套反向镜头桥接并保留当前选择。`矩侨` 路径继续承载柔性、小点距、可死折的传感器电影式首屏，`ShroomJourneySection` 保留蘑菇 loading、床垫三连击、SOS 粒子通知及床垫参数联动。开发态由 Vite 提供前端服务，生产态由 Express 托管 `dist/public` 静态资源。
...
| 3D 渲染    | Three.js                                   | `client/src/components/hero/SensorScene.tsx` + `client/src/components/sections/ShroomJourneySection.tsx` + `client/src/components/effects/SystemScenePreview.tsx` |
| 2D 特效    | Canvas 2D                                  | `PressureParticleField` 复用原 HTML 的规则点阵、双压力波与鼠标响应波纹                                                                                            |
| 路由       | Wouter                                     | SPA 路由切换                                                                                                                                                      |
| 样式       | Tailwind CSS 4 + scoped CSS                | 全局使用 `index.css`，HTML 门户样式集中在 `ShroomPortalHome.css`                                                                                                  |
...
| 目录                             | 主要功能                                                                                                                                |
| :------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------- |
| `client/src/pages`               | 页面级路由组件，目前首页核心是 `Home.tsx`，负责 HTML 风格入口、矩侨演示与 Shroom 剧情之间的模式切换                                     |
| `client/src/assets`              | 本地前端资源，当前包含从原 HTML 恢复的 SHROOM 白色品牌标志数据                                                                          |
| `client/src/components/effects`  | 首页非业务视觉特效组件，当前包含 `PressureParticleField` 压力点阵背景和 `SystemScenePreview` 系统模型粒子预览                           |
| `client/src/components/hero`     | 首页主舞台，包含滚动容器、Three.js 场景、HUD 和传感器数据                                                                               |
| `client/src/components/sections` | 首页场景组件，包含 `ShroomPortalHome.tsx` 的品牌首页/系统弹窗，以及 `ShroomJourneySection.tsx` 的蘑菇 loading / 床垫拍打 / SOS 粒子剧情 |
...
graph TD
  ...
  D --> R[components/sections/ShroomPortalHome.tsx]
  R --> V[components/effects/PressureParticleField.tsx]
  R --> W[components/effects/SystemScenePreview.tsx]
  ...
  W --> N
  W --> O
  W --> P[model/chair3.glb]
  W --> Q[model/jiqirenGggg.fbx]
...
1. 首页渲染流程
   `main.tsx` 挂载 `App`，`wouter` 将 `/` 路由到 `Home`；`Home` 默认渲染 `ShroomPortalHome`，并保留到 `矩侨` 的 `HeroSensorSection` 和 `Shroom` 的 `ShroomJourneySection` 的本地模式切换。
2. HTML 首页与系统列表弹窗
   `ShroomPortalHome` 以原 HTML 的 class 和 DOM 层级实现顶栏、Hero、访问密钥区、行业卡片、页脚与反馈面板；`PressureParticleField` 根据容器尺寸绘制原稿中的规则点阵，并由两组移动压力波和鼠标指针波纹控制亮度与半径。主页蘑菇宿主占满 `100dvh`，`SystemScenePreview` 在模型点云之外使用单个 `BufferGeometry + Points` 绘制 720 个低密度远景粒子，并通过首页专用 FOV 和注视点把蘑菇放大、偏右构图；交互层保持在粒子后方可读，面板只用低透明渐变和轻量模糊提供文字对比。GSAP 时间线按顺序播放顶栏、Hero、密钥面板与行业卡片入场，`quickTo` 复用单个 Tween 响应卡片倾斜；系统弹窗使用可正反播放的时间线协调遮罩、面板、列表、场景和关闭叉，减少动态效果模式会直接缩短为零时长。`SystemScenePreview` 在 React DOM 外创建一个由组件负责销毁的粒子根节点和 WebGL renderer，并根据 `host` 属性把该根节点移动到主页蘑菇宿主或弹窗预览宿主；切换宿主不会重建 `livePositions`、renderer 或模型缓存。打开弹窗前通过 `Flip.getState()` 记录粒子根节点，移动宿主后再由 `Flip.from()` 完成共享元素转场，同时把 `activeScene` 从蘑菇切到所选模型，继续按 X 方向波前左右交替扫动。关闭弹窗时再次记录粒子状态，将节点同步切回主页宿主和蘑菇目标，并临时提高主页粒子舞台层级，让反向 Flip 在弹窗淡出期间始终可见；完成后恢复普通层级。“进入该系统”不再对面板、场景或 Canvas 执行 Flip 缩放，而是直接切换全屏布局并由 `ResizeObserver` 重绘 WebGL。切换布局前，粒子引擎把当前模型包围盒中心经过点组旋转和相机投影换算成全局屏幕坐标；布局改变后先按画布高度比例补偿 FOV，再通过 `PerspectiveCamera.setViewOffset()` 把新画布中的投影对齐到旧屏幕坐标，随后在 `1.08s` 内同步插值相机位置、注视点、FOV 和视窗偏移。返回系统列表重复同一捕获与反算流程，因此进入和返回共享一条双向连续镜头桥接。桌面端系统列表独立滚动，移动端改由弹窗整体滚动，页面滚动会在弹窗关闭后恢复。
3. 滚动驱动首页产品片
...
| 2026-08-26 | 融合首页压力点阵粒子               | 将 `shroom-vision-home-effects.html` 的 canvas 压力点阵动画抽成 React 组件，并接入双入口首页背景                             |
| 2026-08-26 | 首页系统小场景预览                 | 将入口页改为系统场景选择器，点击/悬停系统卡片时驱动右侧粒子模型在蘑菇、床、座椅、机器人之间切换                              |
| 2026-08-26 | SOS 弹窗动态关闭叉                 | 为 `ShroomJourneySection` 的 SOS 通知弹窗新增逐步生成的 Three.js 关闭叉动画                                                  |
| 2026-08-26 | HTML 首页与系统列表弹窗            | 恢复参考 HTML 的 SHROOM 首页构图，以粒子蘑菇为背景，并将系统入口改为居中列表弹窗和选中后模型变形                             |
| 2026-08-26 | 原 HTML 门户与 GSAP 动画融合       | 按原 HTML 恢复完整门户结构与压力点阵，增加 GSAP 入场、卡片倾斜、系统弹窗、动态关闭叉，并在弹窗中切换四个粒子模型             |
| 2026-08-27 | 系统两步选择与全屏展示转场         | 悬停仅高亮、单击切换右侧粒子预览，再通过 GSAP Flip 把同一预览连续放大为全屏展示系统；列表支持大量系统独立滚动                |
| 2026-08-27 | 主页与弹窗共享粒子引擎             | 合并主页蘑菇和弹窗预览的两套 WebGL 场景，以同一个粒子根节点跨宿主移动，并增强主页粒子的亮度、饱和度和青蓝光晕                |
| 2026-08-27 | 蘑菇主视觉层级与反向联动           | 保持原首页结构，仅放大并提高桌面蘑菇层级；关闭弹窗时让同一个粒子画布可见地返回首页并同步重新形成蘑菇                         |
| 2026-08-27 | 系统进入改为相机重新取景           | 移除进入系统时的 Canvas Flip 缩放，保留全屏展示布局，并让 Three.js 相机按当前模型包围盒中心平滑切换到居中构图                |
| 2026-08-27 | 系统列表双向粒子镜头桥接           | 进入和返回前捕获粒子中心的真实屏幕投影，布局变化后以视窗偏移和 FOV 补偿保持首帧位置、尺寸连续，再平滑过渡到目标镜头          |
| 2026-08-27 | 首页轻透全屏粒子背景               | 保留门户信息结构，将蘑菇画布扩展为整屏背景并加入远景粒子，减弱密钥区、卡片和模块块的实体感，同时保持文字与控件可读           |
...
| 2026-08-26 | 新增功能 | 新增 `PressureParticleField`，把静态首页原型中的压力点阵粒子背景融合到当前双入口首页                                                                                                                     |
| 2026-08-26 | 新增功能 | 新增 `SystemScenePreview` 并重构入口页为系统小场景选择器，支持四个系统卡片驱动模型粒子预览切换                                                                                                           |
| 2026-08-26 | 视觉优化 | 将 SOS 弹窗右上角关闭叉改为圆环和两段线逐步生成的 Three.js 动画，而不是静态贴图元素                                                                                                                      |
| 2026-08-26 | 优化重构 | 将入口卡片页收回到参考 HTML 的 SHROOM 品牌首页，新增居中系统列表弹窗，并让默认床垫选择从完整蘑菇形态开始变形                                                                                             |
| 2026-08-26 | 优化重构 | 以 `shroom-vision-home-effects.html` 的原始门户 DOM 和视觉为首页基础，使用 GSAP 编排入场、卡片倾斜、系统弹窗及关闭叉，并保留 Three.js 左右扫动粒子 Morph                                                 |
| 2026-08-27 | 新增功能 | 将系统入口改为“单击选择预览 + 按钮进入”的两步流程，以 GSAP Flip 扩展右侧预览为全屏展示区，加入返回列表、漂移粒子背景、长列表滚动和 WebGL 容器尺寸监听                                                    |
| 2026-08-27 | 优化重构 | 将 `SystemScenePreview` 改为可跨宿主移动的常驻粒子引擎，主页到弹窗通过 GSAP Flip 传递同一个 WebGL 画布及实时粒子状态，并强化主页蘑菇和压力点阵的可见度                                                   |
| 2026-08-27 | 视觉优化 | 在不改变首页结构的前提下放大桌面蘑菇并提高其视觉层级，同时为弹窗关闭增加反向共享元素转场，使当前模型粒子返回主页过程中连续变形成蘑菇                                                                     |
| 2026-08-27 | 优化重构 | 移除进入系统时对面板与 Canvas 的 GSAP Flip 缩放，改为全屏重绘后由 Three.js 相机根据当前模型包围盒中心平滑重新取景，并在返回列表时反向恢复预览镜头                                                        |
| 2026-08-27 | 修复缺陷 | 修复系统预览与全屏展示双向切换时粒子位置和尺寸跳帧的问题，改为捕获真实屏幕投影并通过相机视窗偏移、FOV 补偿和三态镜头插值完成连续桥接                                                                     |
| 2026-08-27 | 视觉优化 | 将首页蘑菇粒子宿主扩展到完整视口，增加低密度远景粒子与首页专用镜头构图，并把访问密钥区、行业卡片和内部模块改为低遮挡轻透层，降低页面厚重感                                                               |
